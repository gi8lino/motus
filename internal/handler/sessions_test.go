...
func TestSessionsHandlers(t *testing.T) {
	...
	t.Run("Session steps", func(t *testing.T) {
		store := &fakeStore{sessionStepTimingsFn: func(context.Context, string) ([]db.SessionStepLog, error) {
			return []db.SessionStepLog{{ID: "s1-0", SessionID: "s1", StepOrder: 0}}, nil
		}}
		api := &API{Store: store}
		h := api.SessionSteps()
	...
	t.Run("Complete session", func(t *testing.T) {
		store := &fakeStore{recordSessionFn: func(context.Context, db.SessionLog, []db.SessionStepLog) error { return nil }}
		api := &API{Store: store}
